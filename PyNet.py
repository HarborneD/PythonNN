...
class NeuralNetwork:
	...
	def __init__(self,input_data_path ="",training_data_path = "", weights_data_path = "",init_weight_config =[],shuffle_data=False):

		if(training_data_path != ""):
			self.set_train_data(training_data_path,shuffle_data)

		if(input_data_path != ""):
			self.set_input_data(input_data_path)

		if(len(init_weight_config) != 0 ):
			self.initialise_weights(init_weight_config,weights_data_path)

		elif(weights_data_path != ""):
	...
	def normalize_data(self):
		...
		transposed = self.input_data.T
		

		for x_feature_count in range(0,self.input_data.shape[1]):
			x_values = transposed[x_feature_count].tolist()[0]
			...
			if(range_x == 0):
				output.append(x_values)
			else:
				new_x_values=[]

				for x in x_values:
					new_x_values.append((x-min_x)/range_x)

				output.append(new_x_values[:])
		new_data = np.matrix(output)


		self.input_data = new_data.T	

	def set_train_data(self,training_data_path,shuffle=False):
		self.train_inputs,self.train_outputs = self.import_data(training_data_path,True,shuffle)
	...
	def initialise_weights(self,init_weight_config,save_name=""):
	...
	def import_data(self,file_path,training=False,shuffle_data=False):
		input_list = []
		output_list = []
	...
	def train_net(self,train_iterations,train_rate = 0.15,momentum = 0.1,save_name="",status_interval=100):
		...
		for epochs in range(0,train_iterations):
			...
			if((status_interval!=0) and (epochs+1) % status_interval == 0):
				print("****iteration: "+str(epochs+1)+"****")
				print("Mean Error:" + str(sum(errors)/len(errors)))
		if(save_name !=""):
			self.layer_weights_to_file(save_name)
	...
	def run_net(self,save_name="",normalize_data=True):
		vecfunc = np.vectorize(self.sigmoid)
...
# NN = NeuralNetwork("test_inputs3-2.csv","test_inputs3-2.csv","",layer_config)

# NN.train_net(1000,train_rate = 0.15,momentum = 0.1,save_name="3-2.xml")

# NN.run_net("3-2_output.csv")
